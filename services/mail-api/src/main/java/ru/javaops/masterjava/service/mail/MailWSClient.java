...
public class MailWSClient {
    ...
    public static String sendToGroup(final Set<Addressee> to, final Set<Addressee> cc, final String subject, final String body, final Set<DataHandler> attachment) throws WebStateException {
        log.info("Send to group to '" + to + "' cc '" + cc + "' subject '" + subject + (log.isDebugEnabled() ? "\nbody=" + body : ""));
        String status = WS_CLIENT.getPort().sendToGroup(to, cc, subject, body, attachment);
        log.info("Send to group with status: " + status);
        return status;
    }

    public static GroupResult sendBulk(final Set<Addressee> to, final String subject, final String body, final Set<DataHandler> attachments) throws WebStateException {
