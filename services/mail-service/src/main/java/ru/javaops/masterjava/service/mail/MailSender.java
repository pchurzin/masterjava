...
public class MailSender {
    ...
    static MailResult sendTo(Addressee to, String subject, String body, Set<DataHandler> attachments) throws WebStateException {
        val state = sendToGroup(ImmutableSet.of(to), ImmutableSet.of(), subject, body, attachments);
        return new MailResult(to.getEmail(), state);
    }

    static String sendToGroup(Set<Addressee> to, Set<Addressee> cc, String subject, String body, Set<DataHandler> attachments) throws WebStateException {
        ...
        try {
            ...
            for (DataHandler dh : attachments) {
                ...
                email.attach(dh.getDataSource(), name, name);
            }
